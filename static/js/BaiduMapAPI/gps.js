...
var GPS = {
	...
	// WGS-84 to GCJ-02  
	gcj_encrypt: function (point) {
		if (this.outOfChina(point.lat, point.lng))
			return { 'lat': point.lat, 'lng': point.lng };

		var d = this.delta(point.lat, point.lng);
		return { 'lat': point.lat + d.lat, 'lng': point.lng + d.lng };
	},
	// GCJ-02 to WGS-84  
	gcj_decrypt: function (point) {
		if (this.outOfChina(point.lat, point.lng))
			return { 'lat': point.lat, 'lng': point.lng };
	...
	gcj_decrypt_exact: function (point) {
		var initDelta = 0.01;
		var threshold = 0.000000001;
		var dLat = initDelta, dLng = initDelta;
		var mLat = point.lat - dLat, mLng = point.lng - dLng;
		var pLat = point.lat + dLat, pLng = point.lng + dLng;
		var wgsLat, wgsLng, i = 0;
		while (1) {
			wgsLat = (mLat + pLat) / 2;
			wgsLng = (mLng + pLng) / 2;
			var tmp = this.gcj_encrypt(wgsLat, wgsLng)
			dLat = tmp.lat - point.lat;
			dLng = tmp.lng - point.lng;
	...
	// GCJ-02 to BD-09   //直接返回百度坐标
	bd_encrypt: function (point) {
